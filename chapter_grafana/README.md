...
お使いのブラウザで <http://grafana.vm30.handson.cloudnativedays.jp/dashboards> にアクセスしてみてください。
...
まずは <http://grafana.vm30.handson.cloudnativedays.jp/dashboards> にアクセスし、 `New` ボタンのプルダウンメニューから `New folder` をクリックし、
...
これにより、 <http://grafana.vm30.handson.cloudnativedays.jp/explore> でPromQLを書きこんでメトリクスを表示したり、
...
Grafana側では <http://grafana.vm30.handson.cloudnativedays.jp/alerting/notifications> にアクセスして、
...
<http://grafana.vm30.handson.cloudnativedays.jp/alerting/routes> にアクセスし、 `New child policy` のボタンをクリックします。
...
<http://grafana.vm30.handson.cloudnativedays.jp/alerting/list> にアクセスし、 `New alert rule` のボタンをクリックします。
...
2. Define query and alert condition
  ...
  - `Label filter` ... `host = app.vm30.handson.cloudnativedays.jp`
  - `Operation` ... 以下を順に設定
...
5. Add annotaions
  - `Summary` ... `app.vm30.handson.cloudnativedays.jp has not received requests over 10 times`
  - `Description` ... `app.vm30.handson.cloudnativedays.jp has not received {{ $labels.method }} requests 10 times`

![image](./image/add-alert-rule-5.png)

このアラートは、1分間隔で取得した、 `app.vm30.handson.cloudnativedays.jp` に対するリクエスト数が10以上でなければアラートを発報するというルールになっています。
