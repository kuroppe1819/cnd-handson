...
[http://pyroscope.vm36.handson.cloudnativedays.jp](http://pyroscope.vm36.handson.cloudnativedays.jp)にアクセスしましょう。すでにPyroscope自身のプロファイルが確認できます。
...
[http://pyroscope.vm36.handson.cloudnativedays.jp](http://pyroscope.vm36.handson.cloudnativedays.jp/) の`Comparison View`から、`Baseline time range`で比較元の期間、`Comparison time range`で比較先の期間を選択すると、比較結果が表示されます。
...
spec:
  ...
  rules:
  - host: pyroscope.vm36.handson.cloudnativedays.jp
    http:
      paths:
      - path: /
        pathType: Prefix
