...
spec:
  ...
  rules:
    - host: grafana.vm30.handson.cloudnativedays.jp
      http:
        paths:
          - path: /
            pathType: Prefix
...
spec:
  ingressClassName: nginx
  rules:
    - host: prometheus.vm30.handson.cloudnativedays.jp
      http:
        paths:
          - path: /
            pathType: Prefix
...
ブラウザで  <http://prometheus.vm30.handson.cloudnativedays.jp> と <http://grafana.vm30.handson.cloudnativedays.jp> にアクセスしてみてください。
...
<http://prometheus.vm30.handson.cloudnativedays.jp/graph> にアクセスして、PromQL入力欄に `go_goroutines` と入力してみます。
...
<http://prometheus.vm30.handson.cloudnativedays.jp/alerts>
...
<http://prometheus.vm30.handson.cloudnativedays.jp/targets>
...
<http://prometheus.vm30.handson.cloudnativedays.jp/graph> を開き (またはリロードして)、PromQL入力欄に ngi と入力し、nginx のメトリクスが追加されているのを確認しましょう。※表示に数分かかります。
