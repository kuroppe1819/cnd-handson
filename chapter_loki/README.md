...
`http://grafana.vm30.handson.cloudnativedays.jp/connections/datasources`にアクセスしてください。
...
設定が完了すると、`http://grafana.vm30.handson.cloudnativedays.jp/connections/datasources`にLokiが表示されます。
...
次にログを発生させます。
サンプルアプリは`http://app.vm30.handson.cloudnativedays.jp/`にアクセスしてみてください。
...
`http://grafana.vm30.handson.cloudnativedays.jp/explore`にアクセスします。
...
`http://grafana.vm30.handson.cloudnativedays.jp/alerting/list`にアクセスしてアラートを設定してみましょう！
...
アラートを設定したら`http://app.vm30.handson.cloudnativedays.jp/`にアクセスして、ログを増やしてみましょう。

> [!TIP]
> アラートの発表状況は`http://grafana.vm30.handson.cloudnativedays.jp/alerting/list`から確認できます。
...
`http://grafana.vm30.handson.cloudnativedays.jp/alerting/list?search=datasource:loki`にアクセスし、Moreから「Delete」を選択肢削除しておきます。
