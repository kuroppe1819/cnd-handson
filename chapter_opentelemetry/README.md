...
`http://grafana.vm30.handson.cloudnativedays.jp/explore` に接続し、`system_cpu_time_seconds_total`のメトリクスを確認してみます。
...
spec:
  ...
  ingress:
    enabled: true
    hosts:
    - jaeger.vm30.handson.cloudnativedays.jp
...
デプロイされたJaegerのUIは、`https://jaeger.vm30.handson.cloudnativedays.jp/search`から確認できます。
...
まず、`http://app.vm30.handson.cloudnativedays.jp/` に接続し、一定量のトレースデータを出力します。
`http://jaeger.vm30.handson.cloudnativedays.jp/search` に接続し、Service名に`handson-blue`を指定してみると、トレースデータが確認できます。
